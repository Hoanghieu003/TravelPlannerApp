...
export default class Calendar extends React.Component<CalendarType, StateType> {
  ...
  UNSAFE_componentWillMount() {
    ...
    start =
      moment(firstMonthToRender).add(numberOfMonths, 'months').toDate() < start ? null : start;
    ...
    if (start) {
      const firstMonth = months.find(
        (m: MonthType): boolean =>
          !!start && m.monthNumber === start.getMonth() && m.year === start.getFullYear(),
      );
      ...
      this.setState(
        ...
        () => {
          if (this.listReference && nextStartDate && startDate !== nextStartDate) {
            ...
            if (index !== -1) {
              const nextVisibleMonths = months.map((month: MonthType, i: number) => ({
                ...month,
  ...
  handleViewableItemsChange = (info: ViewableItemsType) => {
    ...
    if (viewableItemsChanged(firstViewableIndex, lastViewableIndex, info)) {
      ...
      firstViewableIndex =
        firstItemVisible && typeof firstItemVisible.index === 'number' ? firstItemVisible.index : 0;
      lastViewableIndex =
