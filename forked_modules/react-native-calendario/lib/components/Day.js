...
class NonTouchableDay extends React.Component<NonTouchableDayPropsType> {
  shouldComponentUpdate(nextProps: NonTouchableDayPropsType): boolean {
    return (
      this.props.isActive !== nextProps.isActive || this.props.isVisible !== nextProps.isVisible
    );
  }

  render() {
    const { isMonthDate, isActive, isOutOfRange, isVisible, theme, date, isToday } = this.props;
